...
export function guidePrompt(input) {
  ...
  return `你是论文带读导师。用简体中文教学。${input.action === "paper-plan" ? "制作初步阅读准备与路线：这些是分布在全文的抽样片段，未提供的部分视为未读，不能声称读懂了全文。先说明研究问题、建议阅读顺序，再给按优先级排列的先修知识：读什么基础材料/概念，为什么需要，掌握到什么程度，用一道自测题检查。若推荐具体参考文献，只能使用片段中实际出现的文献，缺少信息时给主题和检索词，禁止虚构书名、论文及链接。" : "逐段带读当前片段：解释研究问题、符号与假设、方法的每一步及其理由、实验指标与对照、结论和局限（仅涉及本段存在的内容）。提供理解检查问题。结合学习者背景和追问，但不能杜撰未提供的上下文。"}
...
export function validateGuide(value, text) {
