...
async function openBook(id) {
  ...
  try {
    ...
    $("#pdf-zoom-reset").hidden = active.type !== "pdf";
...
$("#pdf-zoom-reset").onclick = () => {
  if (reader?.pdf) {
    reader.zoom((1 - reader.scale) * 10);
    $("#size-label").textContent = "100%";
  }
};
$("#pdf-container").title = "Ctrl / ⌘ + 滚轮：仅缩放 PDF；底部 + / − 也可缩放";
let pdfWheelDelta = 0,
  pdfWheelTimer;
$("#pdf-container").addEventListener(
  "wheel",
  (e) => {
    if (!reader?.pdf || !(e.ctrlKey || e.metaKey)) return;
    e.preventDefault();
    pdfWheelDelta += e.deltaY;
    clearTimeout(pdfWheelTimer);
    const currentReader = reader,
      anchor = { x: e.clientX, y: e.clientY };
    pdfWheelTimer = setTimeout(() => {
      if (
        reader === currentReader &&
        !$("#reader-view").hidden &&
        pdfWheelDelta
      ) {
        reader.zoom(pdfWheelDelta < 0 ? 1 : -1, anchor);
        $("#size-label").textContent = `${Math.round(reader.scale * 100)}%`;
      }
      pdfWheelDelta = 0;
    }, 60);
...
document.addEventListener("keydown", (e) => {
  ...
  if (["+", "=", "-", "0"].includes(e.key)) {
    e.preventDefault();
    if (e.key === "0") $("#pdf-zoom-reset").click();
    else resizeText(e.key === "-" ? -1 : 1);
  }
});
