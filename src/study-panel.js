...
export function createStudyPanel({ icon, refreshIcons, notify, speechUI }) {
  ...
  card.innerHTML = `<div class="translation-heading"><span>${icon("languages")} GPT 翻译</span><div><button id="translation-copy" class="icon-button" aria-label="复制译文" disabled>${icon("copy")}</button><button id="translation-close" class="icon-button" aria-label="关闭翻译框">${icon("x")}</button></div></div><p id="translation-original"></p><div id="translation-result" role="status"></div><div class="translation-support"><button id="translation-retry" class="text-button" hidden>重新翻译</button><button id="translation-account" class="text-button" hidden>连接与登录 ${icon("arrow-up-right")}</button></div><div class="translation-actions"><button data-study-action="analyze" disabled>${icon("network")} 句法解析</button><button data-study-action="explain" disabled>${icon("lightbulb")} 解释</button><button data-study-action="research" disabled>${icon("search")} 进一步搜索</button><button id="translation-speak" aria-label="朗读原文">${icon("volume-2")}</button></div>`;
